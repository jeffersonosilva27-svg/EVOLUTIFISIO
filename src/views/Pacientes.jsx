...
export default function Pacientes({ pacientes, hasAccess, user, navParams }) {
  ...
  const salvarPaciente = async (e) => {
    ...
    try {
      const dadosLimpos = {
        ...
        emergencia: novoPaciente.emergencia || '',
        valor: novoPaciente.valor || '',
        observacoes: novoPaciente.observacoes || ''
      };

      if (editando) {
        await updateDoc(doc(db, "pacientes", editando), dadosLimpos);
        if (pacienteSelecionado && pacienteSelecionado.id === editando) {
            setPacienteSelecionado(prev => ({...prev, ...dadosLimpos}));
        }
        alert("Dados atualizados com sucesso!");
      } else {
        await addDoc(collection(db, "pacientes"), { ...dadosLimpos, dataCadastro: new Date().toISOString(), status: 'ativo' });
  ...
  // ========================================================================
  // JANELA MODAL UNIVERSAL (Garante que abre em qualquer visão da página!)
  // ========================================================================
  const ModalCadastro = mostrarForm && (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
      <div className="bg-white p-8 md:p-10 rounded-[40px] w-full max-w-2xl shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-y-auto custom-scrollbar">
         <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={() => {setMostrarForm(false); setEditando(null);}} className="text-slate-400 hover:text-red-500 bg-slate-100 hover:bg-red-50 p-2 rounded-full transition-colors"><X/></button>
         </div>
         <form onSubmit={salvarPaciente} className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <input required placeholder="CPF" className="w-full border-2 p-4 rounded-xl bg-slate-50 outline-none focus:border-[#00A1FF] font-bold text-slate-700" value={novoPaciente.cpf} onChange={e => setNovoPaciente({...novoPaciente, cpf: e.target.value})} />
            <input required placeholder="WhatsApp" className="w-full border-2 p-4 rounded-xl bg-slate-50 outline-none focus:border-[#00A1FF] font-bold text-slate-700" value={novoPaciente.whatsapp} onChange={e => setNovoPaciente({...novoPaciente, whatsapp: e.target.value})} />
            <input required placeholder="Tel. Emergência" className="w-full border-2 p-4 rounded-xl bg-slate-50 outline-none focus:border-[#00A1FF] font-bold text-slate-700" value={novoPaciente.emergencia} onChange={e => setNovoPaciente({...novoPaciente, emergencia: e.target.value})} />
            {hasAccess(['gestor_clinico', 'admin_fin']) && (
              <input required type="number" placeholder="Valor da Sessão (R$)" className="w-full border-2 p-4 rounded-xl bg-slate-50 outline-none focus:border-[#00A1FF] font-bold text-green-600" value={novoPaciente.valor} onChange={e => setNovoPaciente({...novoPaciente, valor: e.target.value})} />
            )}
            <textarea placeholder="Observações clínicas iniciais..." className="w-full md:col-span-2 border-2 p-4 rounded-xl bg-slate-50 outline-none focus:border-[#00A1FF] h-24 font-medium text-slate-700" value={novoPaciente.observacoes} onChange={e => setNovoPaciente({...novoPaciente, observacoes: e.target.value})} />
            
            <button type="submit" className="w-full md:col-span-2 bg-[#0F214A] text-white py-4 md:py-5 rounded-[24px] font-black text-lg shadow-xl hover:bg-[#00A1FF] transition-all">
  ...
  // ========================================================================
  // VISÃO 1: FICHA DE UM PACIENTE SELECIONADO
  // ========================================================================
  if (pacienteSelecionado) {
    ...
    return (
      <>
        <div className="space-y-6 animate-in slide-in-from-right-4 duration-300 pb-28">
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <button onClick={() => {
                setPacienteSelecionado(null); 
                setTabAtiva('historico');
                setConfirmarExclusao(false); // CANCELA O ESTADO DE APAGAR AO VOLTAR!
                setMostrarForm(false);
            }} className="flex items-center text-slate-500 font-bold hover:text-[#00A1FF] transition-colors">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => dispararAnaliseIA()} className="p-3 bg-[#1a1b1e] text-white rounded-2xl hover:bg-black shadow-lg flex items-center gap-2">
                {carregandoIA ? <Loader2 className="animate-spin" size={18}/> : <Sparkles size={18} className="text-[#00A1FF]"/>}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
              ...
              <div className="flex flex-wrap gap-4 mt-3 text-slate-500 text-xs font-bold uppercase tracking-widest">
                ...
                {hasAccess(['gestor_clinico', 'admin_fin']) && (
                  <span className="bg-[#e5f5ff] text-[#00A1FF] px-3 py-1.5 rounded-xl border border-blue-100">Sessão: R$ {pacienteSelecionado.valor}</span>
                )}
              </div>
            </div>
                   ...
                   <div className="flex items-end gap-1.5 h-12">
                      {historicoEVAReal.map((evo, i) => (
                        <div 
                          ...
                          className="flex-1 bg-[#00A1FF] rounded-t-md opacity-50 hover:opacity-100 transition-opacity relative group cursor-pointer" 
            ...
            {abasDisponiveis.map(tab => {
              ...
              return (
                <button 
                  ...
                  className={`px-6 py-4 flex items-center gap-2 text-sm font-bold transition-all border-b-2 whitespace-nowrap ${tabAtiva === tab.id ? 'border-[#00A1FF] text-[#00A1FF] bg-[#e5f5ff]' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                >
          ...
          <div className="mt-6">
              ...
              <div className="space-y-6">
                 <div className={`p-8 rounded-[32px] border transition-colors ${editandoEvolucaoId ? 'bg-amber-50 border-amber-200' : 'bg-blue-50 border-blue-100'}`}>
                    <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-4">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-xl border border-blue-200 shadow-sm w-full md:w-auto">
                          ...
                          <input type="range" min="0" max="10" className="w-24 cursor-pointer accent-[#00A1FF]" value={metricaPain} onChange={e => setMetricaPain(e.target.value)}/>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-3">
                      <button onClick={salvarEvolucao} className={`${editandoEvolucaoId ? 'bg-amber-600 hover:bg-amber-700' : 'bg-[#00A1FF] hover:bg-blue-600'} text-white px-8 py-3 rounded-xl font-black shadow-lg transition-colors w-full md:w-auto`}>
            ...
            {tabAtiva === 'plano' && (
               <div className="space-y-6 animate-in slide-in-from-bottom-4">
                  <div className="bg-white p-6 md:p-8 rounded-[32px] border border-slate-100 shadow-sm">
                     <h3 className="font-black text-slate-800 mb-6 flex items-center"><Target className="text-[#00A1FF] mr-2"/> Prescrição de Exercícios</h3>
                     ...
                     <form onSubmit={adicionarExercicio} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 mb-8">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                           ...
                           <div className="md:col-span-2">
                             ...
                             <input required type="text" placeholder="Ex: Supino Reto" className="w-full p-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-[#00A1FF] font-bold text-slate-700 text-sm" value={novoExercicio.nome} onChange={e => setNovoExercicio({...novoExercicio, nome: e.target.value})}/>
                           </div>
                           <div className="md:col-span-1">
                             <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1 block">Carga</label>
                             <input type="text" placeholder="Ex: 10kg" className="w-full p-3 bg-white border border-slate-200 rounded-xl outline-none focus:border-[#00A1FF] font-bold text-slate-700 text-sm" value={novoExercicio.carga} onChange={e => setNovoExercicio({...novoExercicio, carga: e.target.value})}/>
                           </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-4 mt-4 items-center justify-between md:justify-start">
                           ...
                           <button type="submit" className="w-full md:w-auto md:ml-auto bg-[#00A1FF] text-white px-6 py-2.5 rounded-xl font-black hover:bg-blue-600 transition-colors shadow-md text-sm">Adicionar ao Plano</button>
                        </div>
                     </form>
                            ...
                            {Object.entries(planoAgrupado).map(([musculo, exercicios]) => (
                               <div key={musculo} className="bg-white border-2 border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                                  <div className="bg-slate-50 border-b border-slate-100 p-3 flex justify-between items-center">
                                     ...
                                     <span className="text-[10px] font-black text-[#00A1FF] bg-blue-50 px-2 py-1 rounded-md">{exercicios.length} exer.</span>
                                  </div>
            ...
            {tabAtiva === 'financeiro' && hasAccess(['gestor_clinico', 'admin_fin']) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-bottom-2">
                 <div className="bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm">
                    ...
                    <button className="w-full mt-6 py-4 bg-[#0F214A] text-white rounded-2xl font-black hover:bg-black transition-colors flex items-center justify-center gap-2"><Download size={20}/> Gerar Fatura PDF</button>
                 </div>
              </div>
            )}
            ...
            {tabAtiva === 'dados' && (
              <div className="space-y-6 animate-in slide-in-from-bottom-4">
                <div className="bg-white p-6 md:p-10 rounded-[32px] border border-slate-100 shadow-sm">
                  <h3 className="font-black text-slate-800 mb-6 flex items-center"><FileText className="mr-2 text-[#00A1FF]"/> Arquivos e Exames</h3>
                  <div className="bg-slate-50 p-6 md:p-10 rounded-[24px] border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-center hover:bg-slate-100 transition-colors">
                    ...
                    <label htmlFor="exame" className="cursor-pointer bg-[#0F214A] text-white px-6 py-4 rounded-xl font-black flex items-center gap-3 hover:scale-105 transition-all shadow-lg text-sm">
                  ...
                  {laudoExame && (
                    <div className="mt-8 bg-blue-50 p-6 md:p-8 rounded-[32px] border border-blue-100 animate-in zoom-in-95">
                      <h4 className="font-black text-blue-900 mb-6 flex items-center gap-2 border-b border-blue-200 pb-4">
                        <Sparkles className="text-[#00A1FF]"/> Laudo Transcrito (IA)
                      </h4>
            ...
            {tabAtiva === 'ia' && (
              <div className="bg-[#0F214A] text-white p-6 md:p-10 rounded-[40px] shadow-2xl relative overflow-hidden min-h-[500px]">
                <div className="relative z-10">
                  <h3 className="text-2xl font-black mb-8 flex items-center gap-4"><Sparkles className="text-[#00A1FF]" size={32}/> Análise do Agente IA</h3>
                  {carregandoIA ? (
                    ...
                    <div className="text-center py-20">
                       ...
                       <button onClick={dispararAnaliseIA} className="bg-[#00A1FF] text-white px-8 py-4 rounded-2xl font-black hover:bg-blue-600 transition-colors shadow-lg">Iniciar Processamento Quanti-Qualitativo</button>
                    </div>
                  )}
                </div>
        ...
        {/* A MÁGICA: O MODAL TAMBÉM VIVE AQUI, PARA PODER SER ABERTO DENTRO DO PACIENTE! */}
        {ModalCadastro}
      </>
    );
  }
  ...
  // ========================================================================
  // VISÃO 2: LISTA GERAL DE PACIENTES
  // ========================================================================
  return (
    <>
      <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300 pb-28">
        <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
          ...
          <button onClick={() => setMostrarForm(true)} className="bg-[#00A1FF] text-white px-8 py-3.5 rounded-2xl font-black shadow-lg shadow-blue-200 hover:scale-105 transition-all flex items-center justify-center gap-2 w-full md:w-auto">
            <Plus size={20}/> Novo Registro
          </button>
        </div>

        <div className="bg-white p-4 rounded-[24px] border border-slate-200 shadow-sm flex items-center focus-within:ring-2 focus-within:ring-[#00A1FF] transition-all">
      ...
      {/* O MODAL VIVE AQUI TAMBÉM, PARA PODER CADASTRAR NOVOS! */}
      {ModalCadastro}
    </>
  );
}