...
import { 
  ...
  AlertCircle, ChevronRight, X, Loader2, 
  TrendingDown, TrendingUp, Archive, DollarSign
} from 'lucide-react';
import { db } from '../services/firebaseConfig';
import { collection, onSnapshot, addDoc, updateDoc, deleteDoc, doc, query, orderBy } from 'firebase/firestore';

export default function Estoque({ user, hasAccess }) {
  ...
  // ADICIONADO: precoCusto e precoVenda no estado inicial
  ...
  const salvarItem = async (e) => {
    ...
    setSalvando(true);
    try {
      // Garantir que os preços sejam salvos como números caso o usuário digite algo inválido
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-black text-[#0F214A] tracking-tight flex items-center gap-3">
            <Package className="text-[#00A1FF]" size={32}/> Controle de Estoque
          </h1>
          <p className="text-slate-500 font-medium mt-1">Gerencie suprimentos, precificação e materiais da clínica.</p>
        </div>
        <button onClick={abrirFormulario} className="bg-[#00A1FF] text-white px-8 py-3.5 rounded-2xl font-black shadow-lg shadow-blue-200 hover:bg-[#0F214A] transition-all flex items-center gap-2">
      ...
      <div className="bg-white p-4 rounded-[24px] border border-slate-200 shadow-sm flex items-center focus-within:border-[#00A1FF] transition-all">
        ...
        <input placeholder="Buscar no estoque..." className="flex-1 outline-none text-slate-700 bg-transparent font-bold" value={termoBusca} onChange={e => setTermoBusca(e.target.value)} />
      </div>
        ...
        {filtrados.map(item => {
           ...
           return (
             <div key={item.id} className={`bg-white p-6 rounded-[24px] border transition-all flex flex-col ${estoqueBaixo ? 'border-red-200 bg-red-50/30' : 'border-slate-200'}`}>
                ...
                {/* ADICIONADO: Exibição dos Preços no Card */}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
          <div className="bg-white p-10 rounded-[40px] w-full max-w-xl shadow-2xl animate-in zoom-in-95 overflow-y-auto max-h-[95vh] custom-scrollbar">
             ...
             <form onSubmit={salvarItem} className="space-y-5">
                ...
                {/* ADICIONADO: Sessão de Precificação */}
